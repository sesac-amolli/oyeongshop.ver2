...
//    List<Product> findAll();                           Entity    pk
    ...
    // prodSalesDist가 YES인 상품을 카테고리별로 불러옴
    @Query("SELECT p FROM Product p WHERE p.prodCategory = :prodCategory and p.prodSalesDist = 'YES'")
    List<Product> findByProdCategoryJPQL(@Param("prodCategory") String prodCategory);
